...
public class GlobalExceptionHandlerTest {
    ...
    void testMethodNotAllowedException() throws Exception {
        mockMvc.perform(post("/api/books/1"))  // Attempting to POST to a path that only supports GET or DELETE
                .andExpect(status().isMethodNotAllowed())  // Expect 405 Method Not Allowed
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.message").value("Method Not Allowed"))  // Check the message field
                .andExpect(jsonPath("$.timestamp").exists())  // Check the timestamp field
                .andExpect(jsonPath("$.details").exists());  // Check the details field
    }
